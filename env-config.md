...
> 技术栈：Next.js 全栈（App Router + API Routes）、PostgreSQL + Prisma ORM、千问/Qwen 多模态模型
> 部署方式：本地 PostgreSQL 开发 + Vercel 生产部署（PostgreSQL），仅 AI 拍照识别外调千问/Qwen
...
|--------|----------|--------|------|--------|----------|
| `DATABASE_URL` | 是 | - | PostgreSQL 数据库连接串。本地开发和生产部署都使用 PostgreSQL，格式 `postgresql://user:pass@host:port/db`。 | `postgresql://user:password@host:port/db?sslmode=require` | 服务端机密 |

> **配置建议**：本地开发可使用本机 PostgreSQL、Supabase、Neon 或 Vercel Postgres。不要再使用 SQLite provider，避免本地/生产行为不一致。
...
| 变量名 | 是否必填 | 默认值 | 说明 | 示例值 | 安全等级 |
|--------|----------|--------|------|--------|----------|
| `QWEN_API_KEY` | 是 | - | 千问/Qwen API 密钥，用于食物拍照识别功能。 | `sk-abc123...` | 服务端机密 |
| `QWEN_API_ENDPOINT` | 否 | `https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions` | 兼容 OpenAI Chat Completions 的千问请求地址。 | `https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions` | 服务端机密 |
| `QWEN_MODEL` | 否 | `qwen-vl-plus` | 调用千问时使用的多模态模型名称。 | `qwen-vl-plus` / `qwen-vl-max` | 客户端可暴露 |
| `QWEN_TIMEOUT_MS` | 否 | `15000` | Qwen API 请求超时时间（毫秒）。超过此时长将触发降级策略，返回 503 错误。 | `15000` / `20000` | 服务端机密 |
| `QWEN_MAX_TOKENS` | 否 | `1024` | 单次请求返回的最大 Token 数。食物识别结果通常较短，1024 足够。 | `1024` | 客户端可暴露 |
| `QWEN_TEMPERATURE` | 否 | `0.2` | 模型采样温度。越低结果越稳定，适合结构化数据识别。 | `0.2` / `0.1` | 客户端可暴露 |
| `QWEN_MAX_IMAGE_SIZE_MB` | 否 | `10` | 允许上传的图片最大大小（MB）。超过此限制将直接返回 422。 | `10` | 客户端可暴露 |
| `QWEN_MAX_IMAGE_DIMENSION` | 否 | `1024` | 图片 base64 编码前，最短边压缩上限（像素）。用于控制请求体大小。 | `1024` | 客户端可暴露 |

> **安全提示**：`QWEN_API_KEY` 必须通过服务端中转调用，严禁在前端代码或浏览器网络请求中直接暴露。所有图片在上传到 Qwen 之前，后端应完成格式校验（MIME 类型、文件大小、内容可读性），且分析完成后立即丢弃，不保留原始图片文件。
...
| `RATE_LIMIT_AUTH_MAX_REQUESTS` | 否 | `10` | 认证相关接口（登录/注册）的严格速率限制。防止暴力破解。 | `10` / `5` | 服务端机密 |
| `RATE_LIMIT_AI_MAX_REQUESTS` | 否 | `20` | AI 识别接口的速率限制。防止滥用 Qwen API 导致额度耗尽。 | `20` / `10` | 服务端机密 |
...
| `MOCK_AI_RESPONSE` | 否 | `false` | 是否启用 AI 识别模拟模式。开启后 Qwen API 调用将返回预设的 mock 数据，用于离线开发测试。 | `true` / `false` | 服务端机密 |
...
# --------------------------------------------
# 3. 千问/Qwen AI 配置（启用 AI 拍照识别时必填）
# --------------------------------------------
QWEN_API_KEY="sk-你的千问 API 密钥"
...
# QWEN_MAX_IMAGE_SIZE_MB=10
# QWEN_MAX_IMAGE_DIMENSION=1024
...
**本地开发 / 生产部署（PostgreSQL）**：在本机 PostgreSQL、Supabase、Neon 或 Vercel Postgres 创建数据库，获取连接串填入 `DATABASE_URL`。

```bash
DATABASE_URL="postgresql://user:password@host:port/db?sslmode=require"
```

### 第四步：申请千问/Qwen API Key

1. 访问阿里云百炼 / DashScope 控制台
2. 注册账号并创建 API Key
3. 将 Key 填入 `QWEN_API_KEY`
4. 确保账户有充足的多模态模型调用额度
...
- [ ] 如启用 AI 拍照识别，`QWEN_API_KEY` 已替换为生产环境专用 Key
